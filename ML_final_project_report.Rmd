---
title: "PREDICTING INFLATION TREND USING NEWSPAPER AND GOOGLE SEARCH DATA"
author: "Donovan Rasamoelison" 
subtitle: "PSTAT 231 - Machine Learning Project" 
date: "2022-11-19"
output: 
  html_document:
   toc: true
...
Please click [here](https://github.com/Donovan-Rasamoelison/Machine_Learning_Project) for the github repository of this project.

# 1 - INTRODUCTION

Computing inflation rate is a long and arduous task, especially in developing countries. From sending surveys to businesses and consumers to processing and analyzing the data, the entire process can take several months in many countries. However, the current inflation projections (or even projections from the last few months) are often necessary for several economic decision making entities. For instance, central banks use inflation projections to make decisions on interest rates and businesses need inflation projections when accounting for the value of their inventories and supplies. This project, by applying various machine learning models, aims to provide a current and accurate prediction of  inflation using  current search data of keywords related to inflation from Google Trend. Specifically, the main challenge is to investigate if it is possible, without using any prior information on inflation as predictor, to predict inflation using only Google Trend keywords. The common approach to this problem would be to estimate the following model: 

$$ Inflation_{t} = \sum_{s=1}^{S}\delta_sInflation_{t-1-s} + X_t \beta + \sum_{r=1}^{R}X_{t-r} \gamma_{r} + \epsilon_t $$
where: 

$t\text{ is the current period } \text{, } t-1 \text{ is the period with the latest inflation data available}$
$Inflation_{t} = \text{percent change in price in month t-1 compared to prices from a year ago}$ 
$Inflation_{t-1-s} = \text{percent change in price in month t compared to prices from a year ago lagged by some s period}$ 
$X_t = \text{a vector of search magnitude from Google Trend of different inflation keywords}$
$X_{t-r} = \text{a vector of search magnitude from Google Trend of different inlfation keywords lagged by some r period}$

This method will only work if the goal is to predict inflation in the current period $t$, where $t-1$ is the period with the latest inflation data available. However, sometimes inflation data is missing for some $k$ periods before the current period $t$ and it is necessary to have a prediction of inflation in periods $t-k+1$, $t-k+2$,..., $t$. Thus, although it is possible to use the estimated values of inflation in period $t-k+1$ to predict inflation in period $t-k+2$, I estimate the model with the lagged values of inflation removed from the predictor set, and the final model estimated will be: 
$$ Inflation_{i} =  X_i \beta + \sum_{r=1}^{R}X_{i-r} \gamma_{r} + \epsilon_i (1)$$
where: 

$i \in (t-k+1, ..., t) \text{ , current period } t \text{ , } \text{latest inflation data available is at some period } t-k$

It is necessary to clarify that this project does not try to predict future inflation, because future search data are not available. This project tries to estimate inflation values before the current period during which the actual inflation values have not been published.
...
This model can be useful in many ways. For instance, because sending and processing inflation surveys can take several months while Google search data are current, this model can be used to predict the inflation trend after the latest inflation data available to the current period, which can be several months in many countries. Moreover, the prediction from this model can also be used to provide inflation estimates from various countries where collecting inflation data is very difficult.

To conduct the primary analysis, I choose the relevant keywords related to inflation using newspaper data. I use scrapped data from more than 300,000 articles between 2015 and 2017 from major US newspapers such as the New York Times and the Washington Post. First, I filter out the articles that contain the word inflation in the content or in the title. Then, using the articles with the word inflation, I compute the most repeated words in the articles and use them as the keywords relevant for inflation. Unsurprisingly, the most repeated words from the articles are mostly economic terms that are widely used when discussing about inflation, such as interest rate, federal reserve, growth, market. 

After identifying the keywords from the articles, I pipe the search data from different countries related to these keywords using Google Trend. The search data related to these keywords are from 2004, the time Google Trend started to record search data, to 2022. Then, I merge the search data with the country-level inflation data that I collected from the OECD website. The original inflation data I collected from the OECD website contains 43 countries, but due to the time constraints in collecting the data from Google Trend, I only conducted the analysis on 9 countries that I selected as representative of different continents.

Finally, I estimate equation (1) using 5 different Machine Learning (ML) models: polynomial regression, Ridge/Lasso regression, KNN regression, boosted tree, and random forest. I am aware that these models need some adjustments to be suitable for time series data; however, I implement them as covered in class because the class did not cover time series ML models. To estimate these ML models, I proceed with the following steps:
...
- using keywords from the local language (Eg: Brazil: Portuguese, Mexico: Spanish) could also greatly improve the prediction accuracy of the models. 
...
Below is a visualization of the different publishers of the articles about inflation. We can see that Reuters has a fairly large number of articles about inflation compared to the other publishers. 

```{r, message = F, warning = F }
article <- readRDS("./saved_Rdata/article.RData")

article %>% count(publication) %>% 
  mutate(publication = reorder(publication,n)) %>%
  ggplot(aes(x = n, y = publication)) +
  geom_col()+
  labs(x = "Count", y = "Publisher ", title = "Publishers Of The Articles Containing The Keyword: INFLATION") +
...
Using the keywords from the articles, I pipe the search magnitude from different countries related to these keywords from Google Trend. To do this, I use the [gTrendsR](https://cran.r-project.org/web/packages/gtrendsR/gtrendsR.pdf) package. I try to pipe the data from the beginning of Google Trend in January 2004 to September 2022. I selected 9 countries for this study, but more countries can be added. The countries selected were: United States, Great Britain, Australia, Canada, South Africa, Brazil, Japan, India, Mexico, and Switzerland.

For a given country, Google Trend gives a search comparison across the keywords if the data for all the keywords are piped at the same time. So, the search magnitude of each keyword from each country needs to be piped one at a time. First, I create a list for each country. The elements of this list is the search data from Google Trend of each keyword.
...
Next, I follow the detailed process below to combine the data each of the country list:

- First, I select the necessary variables (date, hits, keyword, geo) from the keyword data frame contained in the list related to country j.
...
- The steps above are repeated in the for loop for each keyword of country j and the column of hit values are joined together to form a unique dataframe for country j.

- I bind the dataframe of each country by row to form a single dataframe containing all the countries.
...
keyword_plot1 <- keyword_model_data %>%
  ...
  geom_line(data = keyword_model_data %>% filter(geo == "US"), aes( y= inflation , col = "inflation keyword search magnitude")) +
  geom_line(data = keyword_model_data %>% filter(geo == "US"), aes( y = interest_rate , col = "interest rate keyword search magnitude")) +
  scale_color_manual(values = c("inflation rate" = "black", "inflation keyword search magnitude" = "red" , "interest rate keyword search magnitude" = "blue")) +
  guides(color=guide_legend("")) +
  theme_classic() +
  ylab("") +
  geom_vline(aes(xintercept = lubridate::as_date("2020-03-01")), linetype = "dashed") +
...
keyword_plot2 <- keyword_model_data %>%
  ...
  geom_line(data = keyword_model_data %>% filter(geo == "JP"), aes( y= inflation , col = "inflation keyword search magnitude")) +
  geom_line(data = keyword_model_data %>% filter(geo == "JP"), aes( y = interest_rate , col = "interest rate keyword search magnitude")) +
  scale_color_manual(values = c("inflation rate" = "black", "inflation keyword search magnitude" = "red" , "interest rate keyword search magnitude" = "blue")) +
  guides(color=guide_legend("")) +
  theme_classic() +
  ylab("") +
